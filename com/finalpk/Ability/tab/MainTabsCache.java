package com.finalpk.Ability.tab;
...
public class MainTabsCache {
	private static final Map<ThreadGroup, Record> cache = Collections
			.synchronizedMap(new HashMap<ThreadGroup, Record>());
	...
	public static WidgetChild getTab(final MainTabs tab) {
		final Record record = get();
		update(record);
		if (record.index_widget == -1) {
			return null;
			...
			if (record.indices_tabs[index] == -1) {
				for (final WidgetChild widgetChild : widget.getChildren()) {
					...
					if (actions != null && actions.length > 0
							&& actions[0].equalsIgnoreCase(tab.getDescription())) {
